...
def get_statistic(method,stat,Nran,space):
    ...
    for ir in range(1):
        if stat=='mean':
            ...
            print(ir)
            print( [item[ir] for item in xi0] )
            #print( np.mean([item[ir] for item in xi0]) )
...
for space in ['smallscale']:
    for stat in ['mean']:
        for method in ['ran']:#,'ran','rancross','ransplit']:
            for Nran in [2*87**3]:#,8*87**3]:
