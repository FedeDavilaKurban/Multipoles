...
for i in range(niter):
    ...
    rcat = UniformCatalog(nbar=nran*dcat.size/bs**3, BoxSize=[bs,bs,bs], seed=seed[i])
    ...
    ascii.write(xi_l,'/home/fdavila/Proyectos/Multipoles/data/xi_l_{}_{}.txt'.format(nran*dcat.size,seed[i]),overwrite=True,format='no_header')
    

#Leo los xi_l (si no los calcule ya)
xi_l = []
for i in range(niter):
    xi_l.append( ascii.read('/home/fdavila/Proyectos/Multipoles/data/xi_l_{}_{}.txt'.format(nran*dcat.size,seed[i]),names=['xi_0','xi_2','xi_4','xi_6']) )
...
#plt.errorbar(nr,(nr**2)*xi_0bar,yerr=(nr**2)*np.array(xi_0var),label=r'$\overline{\xi_0(r)}$')
#plt.errorbar(nr,(nr**2)*(-np.array(xi_2bar)),yerr=(nr**2)*np.array(xi_2var),label=r'$\overline{\xi_2(r)}$')
#plt.errorbar(nr,(nr**2)*xi_4bar,yerr=(nr**2)*np.array(xi_4var),label=r'$\overline{\xi_4(r)}$')
##plt.xscale('log')
#plt.legend()
#plt.show()
