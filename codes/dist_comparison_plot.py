...
ax[1].set_title('b) Glass',fontsize=fs)
...
plt.loglog(rPk['k'], rPk['power'].real*len(ran),label='Random')
plt.loglog(gPk['k'], gPk['power'].real*len(glass),label='Glass',color=seaborn.color_palette()[2],ls='-.')
plt.loglog(cPk['k'], cPk['power'].real*len(ccvt),label='CCVT',color=seaborn.color_palette()[1],dashes=[5,2])

gk=gPk['k'][:-200]
plt.hlines(1., 5,rPk['k'][-1],linestyles=':',label=r"$P(k) = 1/\bar{n}$")
plt.loglog(gk,10**8*(gk*msep/(2*np.pi))**4,dashes=[3,3,2,2],c='k',alpha=.8,label=r"$P(k)\propto  k^{4}$") 


plt.xlabel(r"$k[\pi/L_{box}]$",fontsize=fs)
...
plt.loglog(rPk['k'], rPk['power'].real*len(ran),label='Random')
plt.loglog(pk1['k'], pk1['p']*16**3,color=colors[0],ls='-')
plt.loglog(pk2['k'], pk2['p']*16**3,color=colors[1],ls='-')
plt.loglog(pk3['k'], pk3['p']*16**3,color=colors[2],ls='-')
plt.loglog(pk4['k'], pk4['p']*16**3,color=colors[3],ls='-')
plt.loglog(pk5['k'], pk5['p']*16**3,color=colors[15],ls='-')
plt.loglog(pk10['k'], pk10['p']*16**3,color=colors[16],ls='-')
plt.loglog(pk20['k'], pk20['p']*16**3,color=colors[17],ls='-')
plt.loglog(pk30['k'], pk30['p']*16**3,color=colors[18],ls='-')
plt.loglog(pk40['k'], pk40['p']*16**3,color=colors[19],ls='-')
...
plt.loglog(pk50['k'], pk50['p']*16**3,label='Zel. Rec.',color=seaborn.color_palette()[3])
