...
N_zr = len(glob.glob(zrdir+'*zelrec*{}*.txt'.format(Nran))) # glob.glob works like "ls"
print('Number of xi_zelrec is: {} (should be 200)'.format(N_zr))
for iN in range(0,2*N_zr,2):
	# "xil" meaning $\xi_l$, aka multipoles
	xil_zr = ascii.read(zrdir+'xi_l_zelrec_{}_{}-{}.txt'.format(Nran,iN,iN+1,sm),names=['xi_0','xi_2','xi_4','xi_6'])
