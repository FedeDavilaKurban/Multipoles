...
#for ir in range(1):
	...
	if ir==0: ax1.set_ylabel(r'$\sigma_{\xi_0(r)}$',fontsize=fs)
	if ir != 0: ax1.set_yticklabels([])
	...
	if ir == 0: ax1.legend(fontsize=lfs)
	...
	if ir==0: ax2.set_ylabel(r'$\sigma_{\xi_2(r)}$',fontsize=fs)
	ax2.set_xscale('log')
	ax2.set_yscale('log')
	...
	if ir==0: ax3.set_ylabel(r'$\sigma_{\xi_4(r)}$',fontsize=fs)
	ax3.set_xscale('log')
	ax3.set_yscale('log')
	ax3.set_xlabel('$N_{ran}$',fontsize=fs)
	ax3.set_ylim(2E-1,1.3E0)
	if ir!=0: ax3.set_yticklabels([])
...
for Nran in [87**3,2*87**3,4*87**3,8*87**3]:
	...
	ax1.set_position([box.x0, box.y0, box.width, box.height*.8])
	#ax1.legend(loc='upper center', bbox_to_anchor=(0.5, 1.25),ncol=3, fancybox=True, shadow=True,fontsize=lfs)
	if Nran==87**3: ax1.legend(fontsize=lfs)
	ax1.tick_params(axis='x',which='both',bottom=False,top=False,labelbottom=False)
